  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isConfigured) {
        ...
        if (error) {
          // Fallback to password log in
          const { error: passError } = await supabase.auth.signInWithPassword({
            email: emailInput,
            password: 'BharatCartUser123!'
          });
          
          if (passError) {
            // Sign up on the fly if user does not exist
            ...
            setSuccessMessage('Welcome! Registration complete. Please verify your email or use OTP below.');
            setAuthMode('otp');
          } else {
          ...
          setSuccessMessage('Magic login link sent to your email! Access your inbox or enter OTP below.');
          setAuthMode('otp');
        }
      } else {
        setAuthMode('otp');
  ...
  const handleSignupSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isConfigured) {
        ...
        setSuccessMessage('Account created! Verify your email or use OTP below.');
        setAuthMode('otp');
      } else {
        setAuthMode('otp');
      }
          ...
          >
            {authMode === 'login' && (
              <form onSubmit={handleLoginSubmit} className="space-y-5">
                ...
                {errorMessage && (
                  <p className="text-red-500 text-xs font-semibold bg-red-50 dark:bg-red-950/20 p-2.5 rounded-xl border border-red-100 dark:border-red-900/30 text-center">
                    {errorMessage}
                  </p>
                )}
                {successMessage && (
                  <p className="text-green-600 dark:text-green-400 text-xs font-semibold bg-green-50 dark:bg-green-950/20 p-2.5 rounded-xl border border-green-100 dark:border-green-900/30 text-center">
                    {successMessage}
                  </p>
                )}
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-primary hover:bg-blue-700 text-white font-bold h-12 rounded-xl transition-all shadow-md shadow-blue-500/10 flex items-center justify-center space-x-1.5 cursor-pointer disabled:opacity-50"
