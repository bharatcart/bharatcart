...
export default function ProductDetailsModal({
  ...
  useEffect(() => {
    if (product) {
      ...
      if (isShopifyConfigured && product.shopifyHandle) {
        shopifyService.getProductByHandle(product.shopifyHandle)
          .then(res => {
            if (res) {
              setDetailedProduct(prev => {
                ...
                return {
                  ...prev,
                  ...res,
                  id: prev.id, // Keep the same React list ID for consistency
          ...
          .catch(err => console.error("Error loading Shopify product details:", err));
      }
    }
  }, [product]);
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-3 mt-3">
                    <div className="flex items-center space-x-1 text-amber-500">
                      ...
                      <span className="text-sm font-semibold text-slate-800 dark:text-slate-200 font-sans">{activeProduct.rating}</span>
                    </div>
                    <span className="text-xs text-gray-400">|</span>
                    <span className="text-xs text-slate-500 dark:text-slate-400 font-mono underline">{activeProduct.reviewsCount} Verified Reviews</span>
                    <span className="text-xs text-gray-400">|</span>
                    <span className="text-xs text-green-600 dark:text-green-400 font-semibold">In Stock ({activeProduct.stock} items)</span>
                  </div>
                  ...
                  <div className="mt-4 p-4 bg-slate-50 dark:bg-slate-800/40 rounded-2xl border border-gray-100 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      ...
                      <div className="flex items-baseline space-x-3 mt-1">
                        <span className="text-3xl font-extrabold text-primary">₹{activeProduct.price}</span>
                        <span className="text-sm text-gray-400 line-through">₹{activeProduct.originalPrice}</span>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-6 bg-blue-50/50 dark:bg-slate-800/20 p-4 rounded-xl">
                    <div className="flex items-center space-x-2 text-xs">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-gray-400">Shipped in {activeProduct.estimatedDeliveryDays} days</p>
                      </div>
                    </div>
                    <div className="flex items-center space-x-2 text-xs">
                    ...
                    <div className="flex items-center space-x-2 text-xs">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-gray-400">{activeProduct.supplierName}</p>
                      </div>
                    </div>
                  </div>
                </div>
